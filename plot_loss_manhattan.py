...
for p in range(0, 800, 100):
    ...
    for i in range(100):
        if p == 0:
            ...
            basic = np.fromfile('output_manhattan/{:02d}_pca_{:d}_basic_loss'.format(i, p))
            better = np.fromfile('output_manhattan/{:02d}_pca_{:d}_better_loss'.format(i, p))
            kmeanspp = np.fromfile('output_manhattan/{:02d}_pca_{:d}_kmeanspp_loss'.format(i, p))
